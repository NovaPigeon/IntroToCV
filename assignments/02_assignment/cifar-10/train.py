...
def MyCELoss(pred, gt):
    ...
    # print(pred.size())
    pred=torch.exp(pred)
    pred_softmax=(pred.T/torch.sum(pred,axis=1)).T
    # print(pred_softmax.size())
    one_hot=(torch.zeros_like(pred_softmax))
    one_hot[list(range(pred.shape[0])),gt]=1
    # print(gt)
    # print(one_hot)
    loss=-torch.sum(one_hot*torch.log(pred_softmax))
    # print(loss)
    # ----------TODO------------
    return loss

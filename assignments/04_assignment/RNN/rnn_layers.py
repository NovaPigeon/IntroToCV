...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    h[:, 0, :], step_cache = rnn_step_forward(x[:, 0, :], h0, Wx, Wh, b)
    cache.append(step_cache)
    
    for t in range(1, T):
        h[:, t, :], step_cache = rnn_step_forward(x[:, t, :], h[:, t-1, :], Wx, Wh, b)
        cache.append(step_cache)
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ##############################################################################
    #                               END OF YOUR CODE                             #
...
def rnn_backward(dh, cache):
    ...
    for t in reversed(range(T)):
        ...
        dx[:, t, :], dprev_h, dWx_step, dWh_step, db_step = rnn_step_backward(dnext_h, step_cache)
        dWx += dWx_step
        dWh += dWh_step
        db += db_step
